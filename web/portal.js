...
function activity(rows,cases){return rows.slice(0,4).map(s=>{const c=cases.find(c=>c.id===s.case_id);return `<button class="portal-activity" data-resume="${E(s.id)}"><span class="portal-activity-icon">${icon(s.phase==='submitted'?'progress':'practice')}</span><span><b>${E(c?.station_label||'Saved station')}</b><small>${E(phaseLabel[s.phase]||s.phase)} · ${E(new Date(s.created_at).toLocaleDateString(undefined,{month:'short',day:'numeric'}))}</small></span><span aria-hidden="true">↗</span></button>`;}).join('')||'<div class="portal-empty">Your encounters will appear here as you practice. Start with a guided encounter whenever you’re ready.</div>';}
...
function home(data){
 ...
 return `<div class="portal-workspace">${heading('Student portal','Your practice desk.','A place for patient encounters, clear teaching, and the next step in your learning.')}<section class="portal-metrics" aria-label="Your practice at a glance"><div><b>${cases.length}</b><span>patient presentations</span></div><div><b>${completedCount(data)}</b><span>completed attempts</span></div><div><b>${coverage}<small> / ${cases.length}</small></b><span>presentations explored</span></div><div><b>${progress.conditions?.independent||0}</b><span>independent completions</span></div></section><div class="portal-dashboard"><div class="portal-main">${resume}<section><div class="portal-section-heading"><h2>Your workspaces</h2><span>Choose how you want to learn</span></div><div class="portal-destinations">${[
  ...
  ['learn','Read an excellent encounter','Follow a complete example and see where each SOAP statement comes from.','Open written teaching','general'],
...
async function render(kind,ctx){
 ...
 else if(kind==='voice'){ctx.view.innerHTML=`<div class="portal-workspace portal-reading">${heading('Listen and speak','Make the conversation yours.','Choose a supported computer voice and preview it before your next encounter.')}<div id="portalVoiceHost"></div><div id="extraPanel">${ctx.voice()}</div></div>`;ctx.wireVoice();mountVoiceSettings();}
 buttons(ctx.view);ctx.view.focus({preventScroll:true});
}
function restoreVoicePanel(){
 voiceUnsubscribe?.();voiceUnsubscribe=null;
 if(previewActive){window.speechSynthesis?.cancel();previewActive=false;}
 if(voicePanel){document.querySelector('header.topbar')?.after(voicePanel);voicePanel.open=false;voicePanel=null;}
}
function mountVoiceSettings(){
 const host=$('#portalVoiceHost'),existing=$('#patientVoiceSettings');if(!host)return;
 if(existing){voicePanel=existing;host.append(existing);existing.open=true;return;}
 const device=window.pcmDeviceSpeech;
 host.innerHTML=`<section class="portal-side-card"><h2>Computer voice</h2><label for="portalDeviceVoice">Voice supported by this browser</label><select id="portalDeviceVoice"><option value="">Use browser/system default</option></select><div class="row" style="margin-top:12px"><button class="btn primary" id="portalVoicePreview">Preview voice</button><button class="btn" id="portalVoiceStop">Stop preview</button><button class="btn ghost" id="portalVoiceRefresh">Refresh voices</button></div><p class="small" id="portalVoiceStatus" role="status"></p><p class="small muted">Default leaves the voice choice to your browser. The browser may not expose the voice selected in macOS Settings; use this list and preview to choose an explicit match. Computer speech does not use an app-owner API key.</p></section>`;
 const select=$('#portalDeviceVoice'),status=$('#portalVoiceStatus');
 const paint=st=>{select.innerHTML='<option value="">Use browser/system default</option>'+st.voices.map(v=>`<option value="${E(v.uri)}">${E(v.name)} · ${E(v.lang)}${v.default?' · browser default':''}</option>`).join('');if(st.missing)select.insertAdjacentHTML('beforeend',`<option value="${E(st.preference.uri)}">${E(st.preference.name)} · currently unavailable</option>`);select.value=st.preference?.uri||'';select.disabled=!st.supported;$('#portalVoicePreview').disabled=!st.supported;status.textContent=!st.supported?'Speech is unavailable here. Typed encounters remain available.':st.missing?'Your saved voice is currently unavailable. Browser default will be used.':st.resolved?'Saved voice: '+st.resolved.name+'.':'Use browser/system default is selected. Preview to check its sound.';};
 voiceUnsubscribe=device?.subscribe(paint);device?.refresh();
 select.onchange=()=>{const r=device.choose(select.value);if(r.error)status.textContent=r.error;else if(!r.saved)status.textContent='Voice changed for this page, but the browser could not save your preference.';};
 $('#portalVoiceRefresh').onclick=()=>device?.refresh();
 $('#portalVoiceStop').onclick=()=>{if(previewActive)window.speechSynthesis?.cancel();previewActive=false;status.textContent='Preview stopped.';};
 $('#portalVoicePreview').onclick=()=>{if(!window.speechSynthesis||!window.SpeechSynthesisUtterance)return;window.speechSynthesis.cancel();const utterance=new SpeechSynthesisUtterance('Hello, doctor. Thank you for seeing me today.');device?.configure(utterance);previewActive=true;status.textContent='Playing your selected computer voice…';utterance.onend=()=>{previewActive=false;status.textContent='Preview finished. Keep this voice or choose another supported voice.';};utterance.onerror=()=>{previewActive=false;status.textContent='Preview could not play. Try another supported voice or continue with text.';};window.speechSynthesis.speak(utterance);};
}
...
function setupNavigation(){
 ...
 const bar=$('.workspace-nav');if(bar)bar.innerHTML=[['home','Home'],['practice','Practice'],['learn','Learn'],['progress','Progress'],['scoring','Scoring']].map(([key,label])=>`<button class="btn ghost sm" type="button" data-destination="${key}">${icon(key)}<span>${label}</span></button>`).join('');
