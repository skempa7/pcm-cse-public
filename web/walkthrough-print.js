...
function figure(path,caption,cls=''){
 const src=safeImage(path);return src?`<figure class="pw-figure ${cls}"><img src="${esc(src)}" alt="${esc(caption)}"><figcaption>${esc(caption)}</figcaption></figure>`:'';
}
export function chooseSimulation(l,manifest={}){
 const exact=manifest.cases?.[l.case_id];
...
export function buildPrintSections(l,manifest={},credits=[]){
 ...
 const photo=(kind,cls='')=>figure(imgs[kind]||imgs.conversation||imgs.examination,caption,cls);
 ...
 const doorway=[header,`<section class="pw-block pw-doorway"><h2>01 · Read the doorway</h2>${(l.doorway.doorway||[]).map(x=>para(x)).join('')}<dl class="pw-vitals">${Object.entries(l.doorway.vitals||{}).map(([k,v])=>`<div><dt>${esc(k)}</dt><dd>${esc(v)}</dd></div>`).join('')}</dl>${(l.doorway.supplied_results||[]).map(x=>label('Supplied '+x.label,x.value)).join('')}<p class="pw-caption">Doorway vitals and supplied results are authorized information. The equipment illustration does not mean the student measured them.</p></section>`,
 ...
 l.timeline.forEach((t,i)=>{
  ...
  let art='';
  if(equipment)art=figure('print-assets/stethoscope.jpg','Stethoscope · HujiStat, public domain. Equipment illustration.','pw-turn-photo pw-equipment');
  else if(first||t.kind==='action')art=photo(t.kind==='action'?'examination':'conversation','pw-turn-photo');
 ...
 const reasoning=[titleBlock('03 · Reason & remember',l.plan.exam),timingNotes];
 ...
 for(const credit of credits)sources.push(`<section class="pw-block pw-source"><h3>${esc(credit.title)}</h3>${para(credit.author+' · '+credit.license)}${para(credit.usage)}<p class="pw-url">${esc(credit.source_url)}</p></section>`);
 return [{title:'Doorway preparation',kind:'preparation',blocks:doorway},{title:'Encounter storyboard',kind:'storyboard',blocks:encounter},{title:'Clinical reasoning & memory',kind:'reading',blocks:reasoning},{title:'Example SOAP note',kind:'reading',blocks:soap},{title:'Documentation evidence',kind:'reading',blocks:evidence},{title:'Recall exercise',kind:'reading',blocks:recall},{title:'Recall explanations',kind:'reading',blocks:answers},{title:'Sources & review',kind:'reading',blocks:sources}];
}
let activeRoot=null;
export function clearWalkthroughPrint(){document.body.removeAttribute('data-walkthrough-print');document.body.removeAttribute('data-walkthrough-preview');document.getElementById('walkthroughPrintToolbar')?.remove();activeRoot?.remove();activeRoot=null;window.pcmPrintReport=null;}
...
function makePage(root,l,title,kind){
 ...
 page.innerHTML=`<header class="pw-page-header"><span class="pw-brand">Chat CSE <i>/</i> ${esc(title)}</span><span>${esc(l.patient.name)} · ${esc(l.variant_label)}</span></header><div class="pw-columns"><div class="pw-column"></div><div class="pw-column"></div></div><footer class="pw-page-footer"><span>${esc(l.case_id)} · ${esc(l.variant_id)} · Written teaching</span><span class="pw-page-count"></span></footer>`;root.append(page);return page;
}
...
function paginate(root,l,sections){
 ...
 for(const section of sections){
  const continueReading=section.kind==='reading'&&previousKind==='reading';
...
export async function prepareWalkthrough(l){
 ...
 const [manifest,onlineCredits,appCredits]=await Promise.all([
  fetch(assetURL('print-assets/manifest.json?v=landscape-1'),{cache:'no-store'}).then(r=>{if(!r.ok)throw Error('The simulation image set is unavailable. Please try again after reconnecting.');return r.json();}),
 ...
 const root=document.createElement('div');root.id='walkthroughPrint';root.setAttribute('aria-hidden','true');document.body.append(root);activeRoot=root;
