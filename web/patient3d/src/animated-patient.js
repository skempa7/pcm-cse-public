...
export function createAnimatedPatient(scene, options = {}) {
  ...
  function desiredPose() { return ['seated','supine','standing','prone'].includes(state.posture) ? state.posture : 'seated'; }
    ...
    if (clipName) {
      ...
      if (group) {
        ...
        if (previous && state.phase === 'encounter' && !state.reducedMotion && (options.postureTransitionMs || 0) > 0) {
          ...
          poseTransition = {kind:crossfade?'reposition':'blend',started:nowMs(),duration:crossfade?420:options.postureTransitionMs,before,after,fromVisuals:options.poseControlValues?.(previous)||{},toVisuals:options.poseControlValues?.(posture)||{}};
  ...
  const controller = {
    ...
    animate(now = nowMs()) {
      ...
      if (poseTransition) {
        ...
        if(poseTransition.kind==='reposition'){
          // A brief visual transition avoids depicting an unverified roll or
          // sliding the body through the table. The recorded action stays authoritative.
          for(const entry of (u<.5?poseTransition.before:poseTransition.after))restore(entry);
          const visuals=u<.5?poseTransition.fromVisuals:poseTransition.toVisuals;
          for(const [name,value]of Object.entries(visuals||{}))write(name,value);
    ...
    regionAt(point, mesh, pickInfo) {
      ...
      if (['supine','prone'].includes(desiredPose())) {
      ...
      const centerX=marks?.chest ? Vector3.TransformCoordinates(marks.chest,root.getWorldMatrix().clone().invert()).x : 0;
      if (Math.abs(p.x-centerX) > .29 || p.y < a.lapY) return 'MSK';
