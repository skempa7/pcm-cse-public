...
import {conversationFrame,transitionConversationFrame,fitPatientFrame} from './patient-framing.js';
...
let fullFrameBounds=null,manualCamera=false;
let cameraPreset='patient',poseCameraUntil=0,poseCameraFrom='seated',navigation=null;
let mpfbPatient=null,mpfbStatus='idle',mpfbMessage='',mpfbGeneration=0,mpfbBodyBuild=null,patientStatusKey='';
...
function patientNotice(status,message,updateAsset=true){
 message=message||'';if(updateAsset){mpfbStatus=status;mpfbMessage=message;}
 if(!publicState.sessionId)return;
 const key=JSON.stringify([publicState.sessionId,status,message]);
 if(key===patientStatusKey)return;patientStatusKey=key;
 send({type:'pcm-patient-status',sessionId:publicState.sessionId,status,message});
 window.pcmPatientStatus?.({status,message});
}
function ensureMPFB(){
 if(!wantsMPFB()){
  if(publicState.sessionId)patientNotice('limited','A 3D patient is not assigned to this saved presentation. Continue with the conversation and Physical Exam controls.',false);
  return;
 }
 ...
 if(mpfbPatient||mpfbStatus==='failed'){
  // The same asset can serve a different attempt. Replay its actual status for
  // that session rather than leaving the new attempt stuck on Preparing patient.
  patientNotice(mpfbPatient?.loaded?'ready':mpfbStatus,mpfbPatient?.loaded?'Patient ready.':mpfbMessage);
  return;
 }
...
function patientFraming(preset=cameraPreset,posture=publicState.posture){
 ...
 if(anchors?.face){
  const frame=conversationFrame(anchors.face,anchors.lap||anchors.abdomen,preset,posture);
  if(preset!=='full'||active.transition)return frame;
  const key=[mpfbGeneration,trialGeneration,posture,publicState.appearance?.outfit].join(':');
  if(fullFrameBounds?.key!==key){
   let min=V(Infinity,Infinity,Infinity),max=V(-Infinity,-Infinity,-Infinity);
   for(const mesh of scene.meshes){
    if(!active.ownsMesh?.(mesh)||!mesh.isVisible||!mesh.getTotalVertices())continue;
    mesh.refreshBoundingInfo(true,true);mesh.computeWorldMatrix(true);
    const box=mesh.getBoundingInfo().boundingBox;min=Vector3.Minimize(min,box.minimumWorld);max=Vector3.Maximize(max,box.maximumWorld);
   }
   if(min.asArray().every(Number.isFinite)&&max.asArray().every(Number.isFinite))fullFrameBounds={key,min,max};
  }
  return fitPatientFrame(frame,fullFrameBounds,canvas.clientWidth/Math.max(1,canvas.clientHeight));
 }
...
function selectView(preset){if(publicState.phase!=='encounter'||entrance||lesson)return;manualCamera=false;cameraPreset=['face','full'].includes(preset)?preset:'patient';try{if(publicState.sessionId)localStorage.setItem('pcm-camera.'+publicState.sessionId,cameraPreset);}catch{}navigation?.setAdjusting(false);poseCameraUntil=0;desiredCamera=patientFraming(cameraPreset);window.pcmCameraPreset?.(cameraPreset);}
function actualPose(){
...
function finishEntrance(){if(!entrance)return;const id=entrance.id;entrance=null;entered=true;syncInteriorVisibility();doorPivot.rotation.y=-1.64;doorSurround.setEnabled(false);framePatient();send({type:'pcm-room-entered',requestId:id,sessionId:publicState.sessionId});}
function animateEntrance(now){if(!entrance)return;if(publicState.reducedMotion){finishEntrance();return;}const u=clamp((now-entrance.start)/1900),s=x=>x*x*(3-2*x);doorPivot.rotation.y=-1.64*s(clamp(u/.40));const walk=s(clamp((u-.25)/.75));const finish=patientFraming();view(Vector3.Lerp(outsideCamera,finish.position,walk),Vector3.Lerp(outsideTarget,finish.target,walk));syncInteriorVisibility();if(u>=1)finishEntrance();}
...
async function boot(){engine=new Engine(canvas,true,{preserveDrawingBuffer:false,stencil:true,antialias:true,doNotHandleTouchAction:true});engine.setHardwareScalingLevel(1/Math.min(devicePixelRatio||1,1.5));scene=new Scene(engine);scene.useRightHandedSystem=true;scene.clearColor=new Color4(.88,.9,.86,1);scene.imageProcessingConfiguration.exposure=.90;scene.imageProcessingConfiguration.contrast=1.16;camera=new ArcRotateCamera('Room camera',0,0,3,patientTarget,scene);view(outsideCamera,outsideTarget);camera.inputs.clear();scene.preventDefaultOnPointerDown=false;scene.preventDefaultOnPointerUp=false;navigation=createCameraNavigation({canvas,camera,allowed:()=>publicState.phase==='encounter'&&!entrance&&!lesson&&!currentPatient()?.transition,constraints:orbitClearances,onChange:active=>window.pcmCameraAdjusting?.(active),onInteraction:()=>{manualCamera=true;desiredCamera=null;poseCameraUntil=0;}});navigation.remember();camera.fov=.65;camera.minZ=.055;
 ...
 const imported=await SceneLoader.ImportMeshAsync('',new URL('../assets/',import.meta.url).href,'encounter-room.glb?v=no-floating-furniture-1',scene,progress=>{if(progress.lengthComputable)document.getElementById('progress').value=progress.loaded/progress.total});for(const name of ['Male_SeatedRoot','Male_SupineRoot','Female_SeatedRoot','Female_SupineRoot'])scene.getTransformNodeByName(name)?.dispose(false,false);
 ...
 let last=0;engine.runRenderLoop(()=>{const now=performance.now();if(now-last<24||document.hidden)return;last=now;scene.render()});new ResizeObserver(()=>{engine.resize();if(publicState.phase==='encounter'&&cameraPreset==='full'&&!manualCamera&&!entrance&&!lesson)desiredCamera=patientFraming();}).observe(canvas);
 let recoveryFrame=null;
 function reportAfterRecoveredFrame(reason){
  if(recoveryFrame)scene.onAfterRenderObservable.remove(recoveryFrame);
  recoveryFrame=scene.onAfterRenderObservable.add(()=>{
   if(!scene.isReady())return;
   // A restored GPU context alone is insufficient: wait for the current
   // patient to be in a rendered scene, unless the view is outside the door.
   const patient=currentPatient();
   if(publicState.phase==='encounter'&&patient?.loaded&&!scene.getActiveMeshes().data.some(mesh=>mesh&&patient.ownsMesh(mesh)&&['PCM_PublicBody','PCM_AnatomicalBody'].includes(mesh.name)&&mesh.isVisible&&mesh.isReady(true)))return;
   scene.onAfterRenderObservable.remove(recoveryFrame);recoveryFrame=null;
   send({type:'pcm-room-restored',sessionId:publicState.sessionId,reason,patientStatus:wantsMPFB()?mpfbStatus:'limited'});
  });
 }
 engine.onContextRestoredObservable.add(()=>reportAfterRecoveredFrame('graphics-context'));
 window.addEventListener('pagehide',event=>{if(!event.persisted)engine.dispose();});
 window.addEventListener('pageshow',event=>{if(event.persisted){engine.resize();reportAfterRecoveredFrame('page-cache');}});
 canvas.addEventListener('webglcontextlost',()=>{if(recoveryFrame)scene.onAfterRenderObservable.remove(recoveryFrame);recoveryFrame=null;send({type:'pcm-unity-error',sessionId:publicState.sessionId,message:'3D context interrupted; use the main encounter controls.'});});
