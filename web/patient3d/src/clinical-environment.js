...
export async function applyClinicalEnvironment(scene, options = {}) {
  ...
  const report = {version: 'polyhaven-clinical-v3', materials: {}, failures: [], environment: false};
  const resources = [];
  ...
  async function material(short, {tint = '#ffffff', repeat = 1, normal = .24} = {}) {
    const maps = await Promise.all([
      ...
      texture(`${short}-normal.png`, true, repeat),
      texture(`${short}-orm.png`, true, repeat),
    ]);
  ...
  const assignments = [
    ['plaster', {tint: '#ffffff', repeat: 3, normal: .055},
  ...
  const environmentTask = new Promise(resolve => {
    ...
    function finish(error) {
      ...
      scene.environmentIntensity = options.environmentIntensity ?? .49;
      ...
      if (key) {
        key.intensity = options.keyIntensity ?? 1.55;
