...
(()=>{
 ...
 async function start(){
  ...
  await navigator.locks.request('pcm-public-engine-v1',{mode:'exclusive',ifAvailable:true},async lock=>{
   ...
   worker=new Worker(new URL('engine-worker.mjs?v=living-clinic-1',base),{type:'module'});
 ...
 document.addEventListener('DOMContentLoaded',()=>{
  ...
  bar.innerHTML='<strong>Free development preview · SOAP feedback may be inaccurate</strong><span id="localEngineStatus" role="status">Starting the local engine…</span><details><summary>About this edition</summary><p>All encounters, notes and grading run on your device. No account, paid AI, hosted backend or payment service. Computer speech uses your browser’s default voice.</p><p><strong>Development build, not study-ready:</strong> the inherited SOAP checker can flag valid statements incorrectly, especially timing/context, refusals and proposed tests. Check feedback against your course rubric; scores are provisional.</p><p>Work is saved in this browser only. Clearing site data deletes it. Use one tab at a time. Browser/device clocks and client source are accessible: this is a practice tool, not secure exam software. The 24 fictional presentations have 72 written paths; demographic choices are an authored practice cohort, not representative clinical coverage.</p><p>Guided and Coached are untimed. Independent practice: 30-minute encounter, 5-minute organization, 20-minute SOAP. Exam rehearsal: course timing of 14-minute encounter + 9-minute SOAP. No private course documents or personal attempts are included.</p></details>';
