...
async function render(route){const token=++request;const previous=window.pcmActiveAttempt?.();try{
 ...
 const view=$('#view');document.querySelectorAll('[data-destination]').forEach(b=>b.setAttribute('aria-current',b.dataset.destination===(kind==='learn'?'cases':kind)?'page':'false'));
...
async function renderMaterials(route){
 ...
 try{
  ...
  if(!cid){
   view.innerHTML=`<div class="workbench materials-library">${heading('Cases & print','Practice together. Study independently.','Choose a presentation, then print only the materials you need.')}<p class="material-intro"><b>Partner practice:</b> student copy for you; patient script and findings for your partner. <b>Individual study:</b> write first, then open the example note.</p><div class="library-tools"><label>Find a case<input id="materialSearch" type="search" placeholder="Cough, knee, rash…"></label><label>System<select id="materialSystem"><option value="">All systems</option>${window.pcmPortal.orderedSystems(data.cases).map(x=>'<option>'+esc(x)+'</option>').join('')}</select></label><span class="small">${data.cases.length} presentations</span></div><div id="materialGrid" class="case-library-groups"></div></div>`;
   const paint=()=>{const q=$('#materialSearch').value.toLowerCase(),system=$('#materialSystem').value,rows=data.cases.filter(c=>(!system||c.system===system)&&`${c.title} ${c.system}`.toLowerCase().includes(q));$('#materialGrid').innerHTML=window.pcmPortal.groupedCaseLinks(rows,c=>`<a class="lesson-card" href="#cases/${esc(c.id)}"><span class="case-card-art">${window.pcmPortal.illustration(window.pcmPortal.family(c.system))}</span><h2>${esc(c.title)}</h2><span class="lesson-foot">${c.variants.length} case path${c.variants.length===1?'':'s'} <b>Choose materials →</b></span></a>`)||'<p role="status">No matching cases. Try another term.</p>';};$('#materialSearch').oninput=paint;$('#materialSystem').onchange=paint;paint();return;
  }
